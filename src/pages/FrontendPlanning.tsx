      ...
      <div className="canvas">
        <div className="layers">
          ...
          <div id="layer-auth" className="layer">
            ...
            {n({ variant: 'gdpr', title: isEn ? '▲ Clerk' : '▲ Clerk', desc: isEn ? 'Managed auth with pre-built React components. Fastest to implement (<1 hour). Multi-tenant support via organizations. No self-hosting required.' : 'Verwaltete Auth mit vorgefertigten React-Komponenten. Schnellste Implementierung (<1 Stunde). Multi-Mandanten-Unterstützung via Organisationen. Kein Selbst-Hosting erforderlich.', tags: [{ text: isEn ? 'MANAGED' : 'VERWALTET', variant: 'alt' }, { text: isEn ? 'FASTEST' : 'SCHNELLSTE', variant: 'app' }], tipTitle: isEn ? 'When to choose Clerk?' : 'Wann Clerk wählen?', tip: isEn ? 'Best choice if you don\'t need Supabase\'s integrated RLS. Clerk handles MFA, device sessions, and has ready-made UI components. Pricing: free tier → $25/mo (MAU-based). Note: EU data residency is a paid add-on — default is US servers. Use ClerkProvider in React.' : 'Beste Wahl wenn Sie Supabase RLS nicht benötigen. Clerk verwaltet MFA, Gerätesessions und bietet fertige UI-Komponenten. Preise: kostenlos → $25/Monat. Hinweis: EU-Datenresidenz ist ein kostenpflichtiges Add-on.' })}
            {n({ variant: 'gdpr', title: isEn ? '▲ Better Auth (OSS)' : '▲ Better Auth (OSS)', desc: isEn ? 'New open-source TypeScript auth library. Framework-agnostic, any database, self-hosted. Full ownership, no vendor lock-in.' : 'Neue Open-Source TypeScript Auth-Bibliothek. Framework-agnostisch, beliebige Datenbank, selbst-gehostet. Vollständige Eigentümerschaft, kein Vendor-Lock-in.', tags: [{ text: 'OSS · NEW', variant: 'quality' }, { text: isEn ? 'SELF-HOSTED' : 'SELBST-GEHOSTET', variant: 'infra' }], tipTitle: isEn ? 'When to choose Better Auth?' : 'Wann Better Auth wählen?', tip: isEn ? 'If you want full auth ownership without Supabase\'s overhead or Clerk\'s pricing. Works with any database, not just Postgres. Type-safe plugins for 2FA, passkeys, organizations. Still maturing — less ecosystem than Supabase Auth or Clerk. Good choice for pure self-hosted setups.' : 'Wenn Sie vollständige Auth-Eigentümerschaft ohne Supabase-Overhead oder Clerk-Preise wollen. Funktioniert mit beliebigen Datenbanken, nicht nur Postgres. Noch reifend — weniger Ökosystem als Supabase Auth oder Clerk.' })}
          </div>
        ...
        <div className="three-col-grid">
          ...
          <div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {[
                  { tool: 'Supabase Auth', model: isEn ? 'Built-in' : 'Integriert', eu: '✓', cost: '€0', note: isEn ? 'Best with Supabase RLS' : 'Beste mit Supabase RLS', color: 'var(--green)' },
                  { tool: 'Keycloak', model: isEn ? 'Self-hosted' : 'Selbst-gehostet', eu: '✓', cost: '€0–75', note: isEn ? 'Enterprise SSO / SAML' : 'Enterprise SSO / SAML', color: 'var(--alert)' },
                  { tool: 'Clerk', model: isEn ? 'Managed' : 'Verwaltet', eu: '±', cost: '€25+', note: isEn ? 'EU residency paid tier' : 'EU-Residenz kostenpflichtig', color: 'var(--gold)' },
                  { tool: 'Better Auth', model: isEn ? 'OSS / Self-hosted' : 'OSS / Selbst-gehostet', eu: '✓', cost: '€0', note: isEn ? 'Any DB, new library' : 'Beliebige DB, neue Bibliothek', color: 'var(--quality)' },
                ].map((row, i) => (
                  <tr key={i}>
